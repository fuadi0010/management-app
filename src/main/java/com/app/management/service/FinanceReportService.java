...
public class FinanceReportService {
        ...
        public Map<String, Object> getPreviewData(LocalDate start, LocalDate end) {
                ...
                List<SalesInvoice> incomes = salesInvoiceRepository.findBySalesStatusAndInvoiceDateBetween(
                                SalesStatus.COMPLETED, startDT, endDT);

                List<Purchase> expenses = purchaseRepository.findByStatusAndPurchaseDateBetween(
        ...
        public void generatePdf(
                ...
                document.add(new Paragraph("Periode: " + start + " s/d " + end, normalFont));
                ...
                for (SalesInvoice s : incomes) {
                        incomeTable.addCell(s.getInvoiceDate().toLocalDate().toString());
                        incomeTable.addCell(s.getInvoiceNumber());
                        incomeTable.addCell("Rp " + s.getTotalSales());
                }
                document.add(incomeTable);
                document.add(new Paragraph("Total Pemasukan: Rp " + totalIncome, headerFont));
                ...
                for (Purchase p : expenses) {
                        expenseTable.addCell(p.getPurchaseDate().toLocalDate().toString());
                ...
                document.add(new Paragraph(
                                "Saldo Bersih: Rp " + totalIncome.subtract(totalExpense),
                                headerFont));
